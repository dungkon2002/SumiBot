export default {
  ...
  shortDescription: "game cờ caro 5x5",
  usages: [
    'caro @tag : @tag người muốn chơi cùng'
  ],
  cooldowns: 5
};
...
import spf from "node-superfetch";
import ps, { dirname } from "path";
import { fileURLToPath } from 'url';
const __dirname = dirname(fileURLToPath(import.meta.url));


export async function handleReply({ client, handleReply, event, api, global, UsersAll }) {
  ;
  ...
  var namedoithu = UsersAll.find(item => item.id == iddoithu).name;
  console.log(iddoithu, namedoithu);
  //=============Check lượt===========//
  if (luotuser != luot) {
  ...
  //===========CANVAS============//
  //vẽ lại boardgame trước sau đẽ vẽ lên background
  const boardgame = await Canvas.loadImage(boardbuffer);//board lấy từ habdleReply
  ...
  var myname = UsersAll.find(item => item.id == senderID).name;
  //==========CHECK WIN OR NOT==============//
  var CHECKWIN = checkWin(x, y, d, d1, toadoX, toadoY, quanco, sizeboard, sectionSize);
  if (CHECKWIN.WIN == true) {
    ctxx.beginPath();
    ctxx.moveTo(CHECKWIN.startX * sectionSize + 360, CHECKWIN.startY * sectionSize + 320 + sectionSize / 2);
    ctxx.lineTo(sectionSize * 5 + CHECKWIN.startX * sectionSize + 360, CHECKWIN.startY * sectionSize + 320 + sectionSize / 2);
    ctxx.lineWidth = 10;
    ctxx.stroke();
...
export async function run({ client, global, event, api, args }) {
  ...
  if (!fs.existsSync(__dirname + "/cache/memory.ttf")) {
    let getfile = (await axios.get("https://download946.mediafire.com/7ai1ujm9nxmg/oxgwuhttx22glnk/SVN-Agency+FB+Bold.ttf", { responseType: "arraybuffer" })).data;
    fs.writeFileSync(__dirname + '/cache/memory.ttf', Buffer.from(getfile, "utf-8"));
  };
  //==============================
  ...
  //if(!sizeboard || sizeboard == NaN) return api.sendMessage("Sai cú pháp:\n$caro <size> <@tag>\nTrong đó:\n•size: là số ô của bàn cờ (10 < size <20)\n•@tag: tag người muốn chơi cùng\nVí dụ: $caro 10 @NTKhang", threadID, messageID);
  // vẽ boardgame và lưu
  ...
  //var background = await Canvas.loadImage("https://i.ibb.co/VJxH5Hd/Pics-Art-06-29-04-11-24.jpg");
  ...
  Canvas.registerFont(__dirname + "/cache/rank/fonts/bold-font.ttf", {
    family: "caro",
    weight: "regular",
    style: "normal"
  });
