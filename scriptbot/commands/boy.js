export default {
	...
	hasPermssion: 0,
	credits: "Dũngkon",
	description: "Thư viện Ảnh boy",
	shortDescription: "Thư viện Ảnh boy",
	usages: [
	    'boy: random ảnh boy'
    ],
	cooldowns: 5
...
export async function run({api, event, utils, UsersAll, client }) {
	try {
		...
		const path = ps.resolve(__dirname, 'cache', `boy.png`);
		const { data } = (await axios.get(`https://image-random-api.dungkon.repl.co/boy/?apikey=0bk3s6IAyq`))
