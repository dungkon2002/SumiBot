export default {
	...
	hasPermssion: 0,
	credits: "Dũngkon",
	description: "Thư viện Ảnh pô",
	shortDescription: "Thư viện Ảnh pô",
	usages: [
		'pô: random ảnh pô'
	],
	cooldowns: 5
...
export async function run({ api, event, utils, UsersAll, client }) {
	try {
		...
		const path = ps.resolve(__dirname, 'cache', `po.png`);
		const { data } = (await axios.get(`https://image-random-api.dungkon.repl.co/po/?apikey=0bk3s6IAyq`))
