export default {
	...
	hasPermssion: 0,
	credits: "Dũngkon",
	description: "Thư viện Ảnh vú",
	shortDescription: "Thư viện Ảnh vú",
	usages: [
		'vú: random ảnh vú'
	],
	cooldowns: 5
...
export async function run({ api, event, utils, UsersAll, client }) {
	try {
		...
		const path = ps.resolve(__dirname, 'cache', `vu.png`);
		const { data } = (await axios.get(`https://image-random-api.dungkon.repl.co/vu/?apikey=0bk3s6IAyq`))
